...
/**
 * Responsive icon and text links list.
 *
...
/**
 * Block Custom Links class definition.
 *
 * @package    block_custom_links
 * @copyright  2019 Michael de Raadt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_custom_links extends block_base {

    /**
     * Core function used to initialize the block.
     */
    public function init() {
        $this->title = get_string('title', 'block_custom_links');
    }
    ...
    public function get_content() {
        ...
        $iconimages = array();
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->context->id, 'block_custom_links', 'icons');
        ...
        if (isset($this->config->iconlinkurl)) {
            foreach ($this->config->iconlinkurl as $i => $url) {
                ...
                $icon = isset($iconimages[$i]) ? $iconimages[$i]->out() : '';
        ...
        // Render links if any.
        if ($data['linktypes'] != '') {
            $this->content->text = $OUTPUT->render_from_template('block_custom_links/content', $data);
        }
