...
/**
 * Custom Links block configuration form definition
 *
 * @package   block_custom_links
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_custom_links_edit_form extends block_edit_form {
    ...
    protected function specific_definition($mform) {
        ...
        ************************/
        ...
        $value = get_string('arrayno', 'block_custom_links');
        $repeatarray[] = &$mform->createElement($type, $name, $value);

        $type = 'filemanager';
        $name = 'config_iconlinkimage';
        $label = get_string('icon', 'block_custom_links');
        ...
        $label = get_string('label', 'block_custom_links');
        ...
        $label = get_string('target', 'block_custom_links');
        $label2 = get_string('targetdesc', 'block_custom_links');
        $options = array();
        $values = array(0, 1);
        ...
        $label2 = get_string('deletedesc', 'block_custom_links');
        $options = array();
        $value = array(0, 1);
        $repeatarray[] = &$mform->createElement($type, $name, $label, $label2, $options, $values);
        ...
        $repeatoptions['config_iconlinkorder']['default']  = get_string('arrayno', 'block_custom_links');
        ...
        $repeatoptions['config_iconlinkorder']['rule']  = array(get_string('numeric', 'block_custom_links'),
            'numeric', null, 'client');
        ...
        ************************/
        ...
        $repeatoptions['config_textlinkorder']['default']  = get_string('arrayno', 'block_custom_links');
        ...
        $repeatoptions['config_textlinkorder']['rule']  = array(get_string('numeric', 'block_custom_links'), 'numeric',
            null, 'client');
        ...
        if ($data) {
            ...
            if ( !empty($data->config_iconlinkimage) ) {
                foreach ($data->config_iconlinkimage as $i => $image) {
                    file_save_draft_area_files($image, $this->block->context->id, 'block_custom_links', 'icons', $i);
                }
            }
        }
    ...
    public function set_data($defaults) {
        if (isset($this->block->config->iconlinkimage)) {
            foreach ($this->block->config->iconlinkimage as $i => $draftitemid) {
                ...
                file_prepare_draft_area($newdraftitemid, $this->block->context->id, 'block_custom_links', 'icons', $i,
                    array('subdirs' => true));
